...
import CurrencyWorker from "../../db/CurrencyWorker";
import _ from "lodash";
import { Color, Languagegs, withTheme } from "../../common";
...
class CurrencyPicker extends PureComponent {
  ...
  _renderOptions = (option, selected, onSelect, index) => {
    ...
    return (
      <TouchableOpacity
        ...
        <Text style={[!selected && { fontWeight: "100" }]}>({option.name})</Text>
      </TouchableOpacity>
    );
  };
  ...
  render() {
    ...
    return (
      <View>
        <Text >{Languagegs.SelectCurrency}</Text>
        <RadioButtons
